...
int main()
{
    ...
    {
        doAction(state); //L'action du present etat est effectuee a chaque cycle

        if ((estAppuye()) && state = INIT) //Si le bouton-poussoir est enfonce et qu'on est dans INIT, on change a l'etat S1
        {
            _delay_ms(10);
            if (estAppuye())
            {
        ...
        else if (!(estAppuye()) && state = S1) //Si le bouton-poussoir est relache et qu'on est au state S1, on change a l'etat S2
        {
            _delay_ms(10);
            if (!(estAppuye()))
            {
        ...
        else //A partir de l'etat S2, on change d'etat a chaque tour de boucle
        {
            stateSwitch(state);
        }

...
{
    uint8_t premierePosition = 45; //Premiere position donnee pour les notes MIDI
    
    uint16_t frequences[] = {110, 117, 123, 131, 139, 147, 156, 165, 175, 185, 196, 208, 220, 233, 247, 262, 277, 294, 311, 330, 349, 370, 
                                    392, 415, 440, 466, 494, 523, 554, 587, 622, 659, 698, 740, 784, 831, 880};

    uint8_t positionDansTableau = position - premierePosition;

    if(positionDansTableau > 36)
        return;
    ...
    uint16_t compteur = F_CPU / 256; //Comportement du compteur selon le prescaler de 256
...
void arreterSon()
{
    TCCR0A &= ~(1 << COM0A1) & ~(1 << COM0A0); //On deconnece le OC0A pour un retour d'utilisation normal du port
}